...
namespace WFSolution.SetPermissionWF
{
    public sealed partial class SetPermissionWF : SequentialWorkflowActivity
    {
        public SetPermissionWF()
        {
            InitializeComponent();
        }
                ...
                using (SPSite site = new SPSite(siteURL))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        ...
                        if (list.GetItems(query).Count > 0)
                        {
                            ...
                            string str4 = (item1["PermissionField"] != null) ? item1["PermissionField"].ToString() : "Contract";
                            ...
                            lineNumber = 8;
                            this.SetListItemPermission(item, userId, 0x40000002, true);
                            this.SetListItemPermission(item, num4, 0x40000002, false);
                            if (num5 != 0)
                            {
                                this.SetListItemPermission(item, num5, 0x40000002, false);
                            }
                            if (num6 != 0)
                            {
                                this.SetListItemPermission(item, num6, 0x40000002, false);
                            }
                            this.SetListItemPermission(item, 0x33, 0x40000002, false);
                            this.SetListItemPermission(item, 0x34, 0x40000003, false);
                            foreach (int num8 in list5)
                            {
                                this.SetListItemPermission(item, num8, 0x40000002, false);
                                lineNumber = 9;
                            }
                            foreach (int num9 in list6)
                            {
                                this.SetListItemPermission(item, num9, 0x40000003, false);
                            }
                            lineNumber = 10;
                        }
                    }
